...
/**
 * Largest unsuffixed hour accepted by `t()`. Schedule minutes may run into the
 * following campus day (validation caps intervals at 2880 minutes = 48:00), so
 * post-midnight service such as "24:05" or "25:30" stays parseable.
 */
const MAX_EXTENDED_HOUR = 47;
 ...
 * run past midnight up to `MAX_EXTENDED_HOUR` for post-midnight service. Anything else throws.
 */
export function t(text: string): number {
  ...
  if (ap) {
    ...
    if (ap === 'pm' && h < 12) h += 12;
    if (ap === 'am' && h === 12) h = 0;
  } else if (h > MAX_EXTENDED_HOUR) {
    throw new Error(`Bad time: ${text} (hour must be 0-${MAX_EXTENDED_HOUR})`);
  }
  return h * 60 + min;
}
